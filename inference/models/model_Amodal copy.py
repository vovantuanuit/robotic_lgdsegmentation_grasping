...
from grasp_model_amodal import LanguageGraspModel, ResidualBlock
def make_divisible(
    v: Union[float, int],
...
class MobileViT(LanguageGraspModel):
    """
    ...
    def __init__(self, model_cfg: Dict, num_classes: int = 1000):
        ...
        self.conv4 = nn.ConvTranspose2d(240, 120, kernel_size=4, stride=2, padding=1,
                                        output_padding=1)
        self.bn4 = nn.BatchNorm2d(120)

        self.conv5 = nn.ConvTranspose2d(120,32, kernel_size=2, stride=3, padding=2,
                                        output_padding=1)
        self.bn5 = nn.BatchNorm2d(32)

        self.conv6 = nn.ConvTranspose2d(32, 32, kernel_size=2, stride=6, padding=8)
        self.pos_output = nn.Conv2d(in_channels=32, out_channels=1, kernel_size=3)
        self.cos_output = nn.Conv2d(in_channels=32, out_channels=1, kernel_size=3)
        self.sin_output = nn.Conv2d(in_channels=32, out_channels=1, kernel_size=3)
        self.width_output = nn.Conv2d(in_channels=32, out_channels=1, kernel_size=3)
        ...
        pretrained_amodal ='/home/tuanvovan/Documents/grasp-amodal/inference/models/UnseenObjectClustering/data/checkpoints/seg_resnet34_8s_embedding_cosine_rgbd_early_sampling_epoch_16.checkpoint.pth'
        network_name = 'seg_resnet34_8s_embedding'
        num_classes = 2
        train_num_units = 64
        self.amodel_seg = self.load_amodel_segment(pretrained_amodal,network_name,num_classes,train_num_units,device)
    ...
    def forward(self, x_in: Tensor, prompt, query) -> Tensor:
        x = self.conv_1(x_in)
        x = self.layer_1(x)
        x = self.layer_2(x)
        ...
        x = self.convset2(x)
        x = self.transformer_encoder(x)
        # print('x_featyres',x.shape)

        device = x.device
        y_feats = self._encode_text(query, device=device)
        y_feats = self.y_flatten(y_feats)
        y_feats = y_feats.unsqueeze(2).expand(-1, -1, 7).unsqueeze(1).expand(-1, 240, -1, -1)

        amodal_prob_masks = self.amodel_seg(x_in,None)
        prob_map_mask_fused_image = torch.cat([x_in,amodal_prob_masks],dim=1)
        # print("y_feature:",y_feats.shape)
        prob_map_mask_fused_image = torch.cat([x_in,amodal_prob_masks],dim=1)
        x_mask = self.conv_1_fuse(prob_map_mask_fused_image)
        x_mask = self.layer_1(x_mask)
        x_mask = self.layer_2(x_mask)

        x_mask = self.layer_3(x_mask)  # 下采样率8，输出维度：52x52x96
        x1_mask = self.layer_4(x_mask)  # 下采样率16，输出维度：26x26x128
        x2_mask = self.layer_5(x1_mask)  # 下采样率32，输出维度：13x13x160

        x_mask = self.conv_downsampling1(x_mask)
        x_mask = torch.concat([x_mask, x1_mask], dim=1)
        x_mask = self.convset1(x_mask)
        x_mask = self.conv_downsampling2(x_mask)
        x_mask = torch.concat([x_mask, x2_mask], dim=1)
        x_mask = self.convset2(x_mask)
        x_mask = self.transformer_encoder(x_mask)
...
if __name__ == '__main__':
    ...
    # print(x.shape)
    print(model(x,'temp','grasp a bowl').shape)

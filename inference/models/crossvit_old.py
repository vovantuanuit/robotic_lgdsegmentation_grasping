...
class condition_layer_langue(nn.Module):
    def __init__(self, image_size, channels, num_classes, patch_size_small = 8, patch_size_large = 8, small_dim = 50,
                 large_dim = 50, small_depth = 1, large_depth = 4, cross_attn_depth = 1, multi_scale_enc_depth = 3,
    ...
    def forward(self, x_s_langue,x_vision):

        xs = self.to_patch_embedding_small(x_s_langue)
        print(xs.shape)
        b, n, _ = xs.shape
        ...
        for multi_scale_transformer in self.multi_scale_transformers:
            xs, xl = multi_scale_transformer(xs, x_vision)
        # print(xl.shape)
        # print(xs.shape)
...
class CrossViT(nn.Module):
    ...
    def forward(self, xa,xb):
        ...
        xs = self.to_patch_embedding_small(xa)
        print(xs.shape)
        b, n, _ = xs.shape
        ...
        xl = self.to_patch_embedding_large(xb)
        b, n, _ = xl.shape
...
if __name__ == "__main__":
    ...
    img = torch.ones([1, 128, 56, 56])
    
    model = CrossViT(56, 128, 1000)

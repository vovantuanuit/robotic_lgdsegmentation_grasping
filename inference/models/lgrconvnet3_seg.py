...
class GenerativeResnet(LanguageGraspModel):
    ...
    def __init__(self, input_channels=4, output_channels=1, channel_size=32, dropout=False, prob=0.0, clip_version='ViT-B/32'):
        ...
        self.conv1_fused = nn.Conv2d(67, channel_size, kernel_size=9, stride=1, padding=4)
        self.bn1_fused = nn.BatchNorm2d(channel_size)
        ...
        self.conv8 =  nn.ConvTranspose2d(1, 1, kernel_size=4, stride=2, padding=1,
                                        output_padding=1)
        ...
        pretrained_amodal = '/home/tuanvovan/Documents/grasp-amodal/inference/models/UnseenObjectClustering/data/checkpoints/seg_resnet34_8s_embedding_cosine_rgbd_add_sampling_epoch_16.checkpoint.pth'
    ...
    def forward(self, x_in, prompt, query):
        ...
        print('mask: ',amodal_prob_masks.shape)
        processed_mask = []
        ...
        print('map_fused: ',prob_map_mask_fused_image.shape)
        x_mask = F.relu(self.bn1_fused(self.conv1_fused(prob_map_mask_fused_image)))
        ...
        # print('up',x.shape)
        x = F.relu(self.bn7(self.conv7(out_fusion)))
        print('up1',x.shape)
        ...
        print('pos output',width_output.shape)
